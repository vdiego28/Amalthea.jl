use crate::cuda::{GpuBuffer, cublasOperation_t, get_cublas_api, get_gpu_context};
...
/// `J_order(x)`, `order >= 0`, via downward Miller recurrence — docs/dev/BACKLOG.md
...
pub fn jn(order: i32, x: f64) -> f64 {
    ...
    let m_start = {
        ...
        if m.is_multiple_of(2) { m } else { m + 1 }
    };
...
impl Qdht {
    ...
    fn get_or_init_gpu_buffers(&self) -> Option<&GpuBuffers> {
        get_gpu_context()?;
        self.gpu_buffers
            .get_or_init(|| GpuBuffers::new(self.n_r, &self.t_matrix).ok())
            .as_ref()
    }
    ...
    pub fn transform(&self, input: &[Complex<f64>], output: &mut [Complex<f64>]) {
        ...
        if let Some(gpu_buffers) = self.get_or_init_gpu_buffers()
            && let Ok(ctx) = crate::cuda::activate_context()
            && let Ok(cublas) = get_cublas_api()
        {
