...
- **GPU acceleration is real and does run**, but through the native-port's own `CudaNativeSim`/`cuda.rs` (`init_gpu_context`, opt-in via `AMALTHEA_USE_RUST_CUDA_NATIVE=1`), which never touches `dispatch.rs` either. The supported physics is mode-averaged RealGrid/EnvGrid Kerr and matching SDO Raman with at most **64 flattened oscillators** (including N₂ rotation: 49, and rotation+vibration: 50), plus mode-averaged EnvGrid intermediate-broadening `:SiO2` Raman through resident r2c/c2r convolution when explicitly forced with `AMALTHEA_NATIVE_GPU=on`; Plans 08 and 09 support narrow radial RealGrid/EnvGrid scalar-Kerr paths, Plan 10 adds radial RealGrid PPT plasma, and Plan 11 adds radial RealGrid thresholded ADK, all with resident QDHT and per-column transforms under explicit `on`. PPT/thresholded-ADK plasma is RealGrid-only because the CUDA EnvGrid RHS has no plasma implementation; unthresholded ADK (`threshold=false`) remains CPU-only. Kerr-only measured on real hardware (RTX 5060 Ti) at **~5-27x faster above n≈16k RealGrid points** and **~3.3-4.0x at the retained n=32768 EnvGrid threshold** (FFT-dominated, embarrassingly parallel); `AMALTHEA_NATIVE_GPU=off/on/auto` (Julia `Config.jl`) dispatches RealGrid Kerr at n≥16384 and EnvGrid Kerr at n≥32768. Production-shaped Raman measurements stayed below the 1.4x retention bar, so all Raman remains CPU-native under `:auto`; radial `:auto` is also disabled, so use `on` for the explicit CUDA correctness/experiment route.
