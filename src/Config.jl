...
for the same env var (`Ionisation.jl`'s `LUNA_USE_RUST_NATIVE` read used
...
- `native`: `LUNA_USE_RUST_NATIVE`, default `true` — the native-port
  resident stepper (Phase 8 default-flip).
- `cuda_native`: `LUNA_USE_RUST_CUDA_NATIVE` — GPU-resident stepper opt-in.
- `stepper`: `LUNA_USE_RUST_STEPPER` — the older per-kernel callback stepper.
- `ionisation`: `LUNA_USE_RUST_IONISATION` — PPT ionization LUT kernel.
- `raman`: `LUNA_USE_RUST_RAMAN` — time-domain Raman SDO ADE kernel.
...
`LUNA_*` backend toggle should be added, so its default and truthiness
convention are visible in one spot instead of scattered across call sites.
"""
function backend_config()
    BackendConfig(
        native       = _truthy("LUNA_USE_RUST_NATIVE", "1"),
        cuda_native  = _truthy("LUNA_USE_RUST_CUDA_NATIVE", "0"),
        stepper      = _truthy("LUNA_USE_RUST_STEPPER", "0"),
        ionisation   = _truthy("LUNA_USE_RUST_IONISATION", "0"),
        raman        = _truthy("LUNA_USE_RUST_RAMAN", "0"),
        dispersion   = _truthy("LUNA_USE_RUST_DISPERSION", "0"),
