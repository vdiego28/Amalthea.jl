...
@testitem "Julia-Rust Phase 4 Integration (Scans & I/O)" tags=[:rust] begin
    ...
    @test idx0 in (0, -2) # File locking can cause -2 on Windows
    ...
    @test idx1 in (1, -2) # File locking can cause -2 on Windows
    ...
    try
        HDF5.h5open(qfile, "r") do file
            ...
            @test qdata[1] in (2, 0)  # Completed successfully or never written
            @test qdata[2] in (3, 0)  # Failed or never written
            @test qdata[3] in (2, 0)  # Completed successfully or never written
        end
    catch e
        if !Sys.iswindows()
            rethrow(e)
