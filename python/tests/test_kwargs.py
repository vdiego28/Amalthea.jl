import pytest
from amalthea._kwargs import translate_kwargs
...
def test_translate_kwargs_duplicate_lambda(monkeypatch):
    ...
    monkeypatch.setattr("amalthea._kwargs._to_julia_type", lambda k, v: v)
...
def test_translate_kwargs_basic(monkeypatch):
    import amalthea._kwargs
    mock_jl = type('MockJL', (), {'Symbol': lambda x: f"Symbol({x})"})
    monkeypatch.setattr(amalthea._kwargs, "_to_julia_type", lambda k, v: mock_jl.Symbol(v) if k in ("gas", "model", "pulseshape", "polarisation", "plasma", "ramanmodel", "modes") else v)
    kwargs = {"energy": 1.0, "duration": 10e-15}
    out = amalthea._kwargs.translate_kwargs(kwargs)
...
def test_translate_kwargs_modes(monkeypatch):
    import amalthea._kwargs
    mock_jl = type('MockJL', (), {'Symbol': lambda x: f"Symbol({x})"})
    monkeypatch.setattr(amalthea._kwargs, "_to_julia_type", lambda k, v: tuple(mock_jl.Symbol(x) if isinstance(x, str) else x for x in v) if k == "modes" else v)
...
def test_translate_kwargs_duplicate_duration(monkeypatch):
    ...
    with pytest.raises(ValueError, match="Duplicate kwarg"):
        amalthea._kwargs.translate_kwargs(kwargs)
