...
def mock_julia_backend(monkeypatch):
    ...
    monkeypatch.setattr(amalthea._kwargs, "get_julia", mock_get_julia)
    monkeypatch.setattr(amalthea, "LunaOutput", MockLunaOutput)

    # In some module setups, patching `sys.modules` is the only way to intercept nested intra-package imports
    monkeypatch.setitem(sys.modules, "amalthea._julia", MagicMock(get_julia=mock_get_julia))
